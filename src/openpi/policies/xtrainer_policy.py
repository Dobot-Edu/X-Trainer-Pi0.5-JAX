...
def make_xtrainer_example() -> dict:
    """Creates a random input example for the XTrainer policy."""
    return {
...
class XTrainerInputs(transforms.DataTransformFn):
    """Inputs for the XTrainer policy.
...
class XTrainerOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
        # Return the bimanual [left(6+1), right(6+1)] action dimensions.
